...
19. We are going to create a new "data-event-explore view" for viewing / managing data event subscriptions, and viewing the events associated with a particular subscription.

19.1 Menu Navigation.  Navigation to the view will be from a new menu item "Data Events" on the "Explore" menu.

19.2 View Layout.  The view will consist of three sections.  The section on the left is labeled "Data Event Subscriptions", and runs the full length vertically of the view.  The area on the right is divided into two vertically arranged sections.  The top section is labeled "Subscription Builder".  The bottom section is labeled "Data Events".  The view layout should follow the style and feel of the "pv-explore" view with its three sections.  Each of the three data-event-explore view sections is described in more detail below.

19.3 "Data Event Subscriptions" section.  The main purpose of this section is to display the list of data event subscriptions, which is obtained from and kept synchronized with the DpApplication member variable "dataEventSubscriptions".  The main element in this section is a list box displaying the list of DataEventSubscription objects.  The display string for each item in the list should show the PV name, operator, and value for the DataEventSubscription's SubscribeDataEventDetail member (like is done in the subscription-details-component list).  The display string should use a hyperlink that updates the contents of the "Data Events" section when it is selected as described below in section 19.5.  Each item in the list box should also include a button with a trash can icon (like in the "Query PVs" section of the pv-explore view) that cancels the subscription for the selected item by calling DpApplication.cancelDataEventSubscription(), passing the corresponding DataEventSubscription object as a parameter.  That method returns a ResultStatus indicating success or failure of the operation.  If the ResultStatus.isError flag is set, the operation failed and the ResultStatus.msg error message should be displayed in the status bar.  That method modifies the list in DpApplication.dataEventSubscriptions to remove the canceled subscription.  The "Data Event Subscriptions" list displayed in the view should be updated accordingly.

19.4 "Subscription Builder" section.  This section presents a form for adding new data event subscriptions.  The form should contain the same fields used in the "subscription-details-component" form for PV name, operator, and data value, as well as a field labeled "PV Data Type" with the following options (from the IngestionClient.IngestionDataType enum) UINT, ULONG, INT, LONG, FLOAT, and DOUBLE.  It should include a button labeled "Add".  When the "Add" button is clicked, the DpApplication.subscribeDataEvent() should be called with a new SubscribeDataEventDetail object created from the form contents (bound to the subscriptionDetail parameter) and the IngestionDataType enum value coresponding to the value selected in the "PV Data Type" field (bound to the dataType parameter).  That method will add the new subscription to list in DpApplication.dataEventSubscriptions.  The method returns a ResultStatus object reflecting the success or failure of the operation.  If the isError flag in the ResultStatus is set, the operation failed and the error message in ResultStatus.msg should be displayed in the status bar.  Otherwise, the operation was successful and the list of subscriptions displayed in the data-event-explore-view's "Data Event Subscriptions" section should be updated to reflect the new subscription.

19.5 "Data Events" sction.  This section displays a tabular list of data events corresponding to the subscription selected in the "Data Event Subscriptions" section's list of subscriptions.  The list is obtained via the method DpApplication.dataEventsForSubscription(), passing the DataEventSubscription selected in the "Data Event Subscriptions" section as the method parameter.  When the hyperlink for an item in that list is selected, the events for the selected subscription are displayed in the "Data Events" section table.  The table contains two columns, 1) "Event Time" and 2) "Trigger Value". The value in the "Event Time" column should use a hyperlink.  When the hyperlink is clicked, the display should navigate to the data-explore view's "Query Editor" tab, displaying the PV name for the subscription selected in the Data Event Subscriptions view and setting the Query Editor's Begin and End time spinners to the timestamp of the event corresponding to the event selected in the "Data Events" table.  The Query Editor uses the DpApplication member variables pvNames, dataBeginTime, and dataEndTime, respectively, as the models for those display elements so before navigating from the data-event-explore view to the Query Editor, the member variable values in DpApplication should be updated.